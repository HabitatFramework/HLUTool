    ...
    public sealed class NamedPipeWrapper
    {
        ...
        public static bool TryConnectToPipe(string pipeName, string serverName, out PipeHandle handle)
        {
            ...
            try
            {
                ...
                handle.Handle = NamedPipeNative.CreateFile(name, NamedPipeNative.GENERIC_READ | 
                    NamedPipeNative.GENERIC_WRITE, 0, null, NamedPipeNative.OPEN_EXISTING, 0, 0);
        ...
        public static PipeHandle ConnectToPipe(string pipeName, string serverName)
        {
            ...
            for (int i = 1; i <= ATTEMPTS; i++)
            {
                ...
                if (handle.Handle.ToInt32() != NamedPipeNative.INVALID_HANDLE_VALUE)
                {
                    ...
                    {
                        handle.State = InterProcessConnectionState.Error;
                        throw new NamedPipeIOException("Error setting read mode on pipe " + name + " . Internal error: " + 
                            NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
                    }
                }
                if (i >= ATTEMPTS)
                {
                    if (NamedPipeNative.GetLastError() != NamedPipeNative.ERROR_PIPE_BUSY)
                    {
                        ...
                        throw new NamedPipeIOException("Error connecting to pipe " + name + " . Internal error: " + 
                            NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
                    }
                    else
                    {
                        handle.State = InterProcessConnectionState.Error;
                        throw new NamedPipeIOException("Pipe " + name + " is too busy. Internal error: " + 
                            NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
                    }
                }
                else
        ...
        public static PipeHandle Create(string name, uint outBuffer, uint inBuffer, bool secure)
        {
            ...
            try
            {
                for (int i = 1; i <= ATTEMPTS; i++)
                {
                    ...
                    {
                        handle.State = InterProcessConnectionState.Error;
                        throw new NamedPipeIOException("Error creating named pipe " + name + " . Internal error: " + 
                            NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
                    }
                }
            }
        ...
        public static void GetNullDaclSecurityDescriptor(out SECURITY_DESCRIPTOR sd)
        {
            if (NamedPipeNative.InitializeSecurityDescriptor(out sd, 1))
            {
                ...
                {
                    throw new NamedPipeIOException("Error setting SECURITY_DESCRIPTOR attributes. Internal error: " + 
                        NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
                }
            }
            else
        ...
        public static void Connect(PipeHandle handle)
        {
            ...
            {
                handle.State = InterProcessConnectionState.Error;
                throw new NamedPipeIOException("Error connecting pipe. Internal error: " + 
                    NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
            }
        }
        ...
        public static uint NumberPipeInstances(PipeHandle handle)
        {
            ...
            {
                throw new NamedPipeIOException("Error getting the pipe state. Internal error: " + 
                    NamedPipeNative.GetLastError().ToString(), NamedPipeNative.GetLastError());
            }
        }
